...
```bash
./scripts/publish-crates.sh --verify-only --version 0.7.0
```
...
```bash
./scripts/publish-crates.sh --dry-run --version 0.7.0
```
...
```bash
PHANTOM_PUBLISH_CONFIRM=publish-phantom-secrets-0.7.0 \
  ./scripts/publish-crates.sh --publish --version 0.7.0
```
