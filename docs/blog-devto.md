...
published: false
description: "AI coding agents with dotenv access can expose API keys. I built an open-source Rust CLI that replaces managed secrets with phantom tokens and uses an authenticated local proxy."
...
```bash
# 1. After installing the reviewed v0.7.8 GitHub release, initialize the project
$ phantom init
...
Install the checksum-verified `v0.7.8` GitHub release binaries using the
