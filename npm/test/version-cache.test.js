...
assert.strictEqual(parseBinaryVersion("phantom 0.7.2\n"), "0.7.2");
assert.strictEqual(parseBinaryVersion("phantom 0.7.2\r\n"), "0.7.2");
assert.strictEqual(
...
for (const malformed of [
  ...
  "phantom 0.7.01\n",
  "phantom 1.2.3-01\n",
  "phantom 0.7.2-untrusted\nextra\n",
  "prefix phantom 0.7.2\n",
  "phantom-mcp 0.7.2\n",
...
try {
  ...
  assert.strictEqual(validateCachedBinary(paths.binaryPath, paths.manifestPath, {
    execFileSyncImpl: () => {
      executions += 1;
      return Buffer.from("phantom 0.7.2\n");
    },
  }), false);
  ...
  assert.strictEqual(isCachedBinaryStale(paths.binaryPath, {
    execFileSyncImpl: () => Buffer.from("phantom 0.7.2\n"),
  }), false);
