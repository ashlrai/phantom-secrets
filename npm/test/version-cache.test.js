...
assert.strictEqual(parseBinaryVersion("phantom 0.7.1\n"), "0.7.1");
assert.strictEqual(parseBinaryVersion("phantom 0.7.1\r\n"), "0.7.1");
assert.strictEqual(
...
for (const malformed of [
  ...
  "phantom 0.7.01\n",
  "phantom 1.2.3-01\n",
  "phantom 0.7.1-untrusted\nextra\n",
  "prefix phantom 0.7.1\n",
  "phantom-mcp 0.7.1\n",
...
try {
  ...
  assert.strictEqual(validateCachedBinary(paths.binaryPath, paths.manifestPath, {
    execFileSyncImpl: () => {
      executions += 1;
      return Buffer.from("phantom 0.7.1\n");
    },
  }), false);
  ...
  assert.strictEqual(isCachedBinaryStale(paths.binaryPath, {
    execFileSyncImpl: () => Buffer.from("phantom 0.7.1\n"),
  }), false);
