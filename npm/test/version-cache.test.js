...
function writePair(binaryPath, manifestPath, contents) {
  ...
  writePrivateFile(manifestPath, `${JSON.stringify({ version: "0.7.0", sha256 })}\n`, 0o600);
}

assert.strictEqual(parseBinaryVersion("phantom 0.7.0\n"), "0.7.0");
assert.strictEqual(parseBinaryVersion("phantom 0.7.0\r\n"), "0.7.0");
assert.strictEqual(
...
for (const malformed of [
  ...
  "phantom 0.7.00\n",
  "phantom 1.2.3-01\n",
  "phantom 0.7.0-untrusted\nextra\n",
  "prefix phantom 0.7.0\n",
  "phantom-mcp 0.7.0\n",
...
try {
  ...
  assert.strictEqual(validateCachedBinary(paths.binaryPath, paths.manifestPath, {
    execFileSyncImpl: () => {
      executions += 1;
      return Buffer.from("phantom 0.7.0\n");
    },
  }), false);
  ...
  assert.strictEqual(isCachedBinaryStale(paths.binaryPath, {
    execFileSyncImpl: () => Buffer.from("phantom 0.7.0\n"),
  }), false);
