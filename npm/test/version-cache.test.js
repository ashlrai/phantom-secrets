...
function writePair(binaryPath, manifestPath, contents) {
  ...
  writePrivateFile(manifestPath, `${JSON.stringify({ version: "0.7.4", sha256 })}\n`, 0o600);
}

assert.strictEqual(parseBinaryVersion("phantom 0.7.4\n"), "0.7.4");
assert.strictEqual(parseBinaryVersion("phantom 0.7.4\r\n"), "0.7.4");
assert.strictEqual(
...
for (const malformed of [
  ...
  "phantom 0.7.01\n",
  "phantom 1.2.3-01\n",
  "phantom 0.7.4-untrusted\nextra\n",
  "prefix phantom 0.7.4\n",
  "phantom-mcp 0.7.4\n",
...
try {
  ...
  assert.strictEqual(isCachedBinaryStale(paths.binaryPath, {
    execFileSyncImpl: () => Buffer.from("phantom 0.7.4\n"),
  }), false);
