...
test("workflow identity is bound to the tagged release definition and source SHA", () => {
  ...
  const workflowEnv = {
    ...
    GITHUB_WORKFLOW_REF:
      "ashlrai/phantom-secrets/.github/workflows/npm-candidate-acceptance.yml@refs/tags/v0.7.8",
  };
  assert.deepEqual(validateWorkflowIdentity(workflowEnv, config), {
    githubRef: "refs/tags/v0.7.8",
    githubSha: sourceSha,
...
test("prepublication mode rejects burned-version ambiguity and public integrity inputs", () => {
  ...
  assert.throws(
    () =>
      validateCandidateTagState(
        { latest: "0.6.0", "release-candidate": "0.7.8" },
        "prepublication",
        "0.7.8",
        "0.6.0",
        "0.7.4",
  ...
  assert.match(smoke, /spec: localTarballSpec\(tarballPath\)/);
});

test("local tarball specs preserve native Windows 8.3 paths for npm to encode once", () => {
  const windowsTarball =
    String.raw`C:\Users\RUNNER~1\AppData\Local\Temp\phantom-npm-packs\phantom-secrets-0.7.8.tgz`;
  assert.equal(localTarballSpec(windowsTarball), windowsTarball);
  assert.doesNotMatch(localTarballSpec(windowsTarball), /%7E/i);
  assert.equal(
    localTarballSpec("/tmp/phantom-secrets-0.7.8.tgz"),
    "/tmp/phantom-secrets-0.7.8.tgz",
  );
...
test("local package receipts enforce the exact five-file wrapper closure", () => {
  const pack = {
    ...
    version: "0.7.8",
    filename: "phantom-secrets-0.7.8.tgz",
    entryCount: 5,
  ...
  assert.equal(validatePackReceipt([pack], "phantom-secrets", "0.7.8"), pack);
  assert.throws(
    () =>
      validatePackReceipt(
