...
export function verifyTagBinding(tag, expectedCommit, remote = "origin") {
  if (!stableTagPattern.test(tag ?? "") || !shaPattern.test(expectedCommit ?? "")) {
    throw new Error(
      "usage: verify-github-tag-binding.mjs vMAJOR.MINOR.PATCH EXPECTED_COMMIT [REMOTE]",
    );
  }
  ...
  const output = runGit(["ls-remote", "--tags", remote, tagRef, peeledRef]);
  const refs = new Map();
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  const result = verifyTagBinding(process.argv[2], process.argv[3], process.argv[4]);
  console.log(
    `verified annotated remote tag ${process.argv[2]} at commit ${result.peeledCommit}`,
  );
}
