...
This wrapper is version `0.7.9`. Its manifest and this README do not prove npm,
GitHub, or MCP Registry publication. Verify the exact npm version before
installing it:

```bash
npm view phantom-secrets-mcp@0.7.9 version dist.integrity dist.tarball --registry=https://registry.npmjs.org/
npm install --global phantom-secrets-mcp@0.7.9 --registry=https://registry.npmjs.org/
phantom-mcp --version
```
...
identity. If the npm query, release, checksum, or expected `phantom-mcp 0.7.9`
...
Version `0.7.9` records the installed local `phantom mcp serve` command when it
...
- Provider enrollment and live issuance/refresh/renewal/rotation/revocation are
  hard-denied before credential and network access in 0.7.9, including from
...
`phantom-secrets-mcp@0.7.9` with the exact `npm view` command above, then verify
