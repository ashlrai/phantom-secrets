...
async function acquireInstallLock(lockPath, {
  ...
  while (true) {
    try {
      ...
      const heartbeat = () => {
        if (released) throw new Error("phantom-mcp install lock is already released");
        if (heartbeatFailure) throw heartbeatFailure;
        const current = readInstallLock(lockPath, platform);
        if (current.token !== token) {
          throw new Error("phantom-mcp install lock ownership was lost");
        }
        const now = new Date();
      ...
      if (Date.now() >= deadline) throw new Error("timed out waiting for phantom-mcp install lock");
...
async function ensureBinary({
  ...
  try {
    ...
    try {
      ...
      heartbeat();
      console.error(`Installed phantom-mcp to ${paths.binaryPath}`);
      return paths.binaryPath;
    } finally {
